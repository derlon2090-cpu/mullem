(() => {
  ...
  let state = loadState(pageKey, pageConfig.defaults);
  ...
  function renderSidebarNav() {
    return navItems
      .map((item) => {
        ...
        return `
          <a class="workspace-nav-link ${active}" href="${item.href}">
            <span>${item.label}</span>
            <i>${icons[item.icon]}</i>
          </a>
        `;
  ...
  function handlePrimaryAction() {
    if (pageKey === "messages") {
      ...
      const created = {
        id: makeId("conv"),
        title,
        preview: "ابدأ رسالتك هنا",
        time: "الآن",
        messages: [{ role: "assistant", text: `تم فتح "${title}"، ابدأ سؤالك الآن.` }]
      };
      state = { ...state, activeId: created.id, conversations: [created, ...state.conversations] };
      persist();
      render();
      toast("تمت إضافة محادثة جديدة.");
      return;
    }
    if (pageKey === "projects") {
      const title = window.prompt("اسم المشروع", "مشروع جديد");
      if (!title) return;
      state = {
        ...state,
        projects: [{ id: makeId("project"), title, updated: "تحديث الآن", count: 1, icon: "rocket", tone: "purple" }, ...state.projects]
      };
      persist();
      render();
      toast("تمت إضافة المشروع الجديد.");
    ...
    if (pageKey === "library") {
      const title = window.prompt("اسم الملف", "ملف جديد");
      if (!title) return;
      state = {
        ...state,
        files: [{ id: makeId("file"), title, type: "PDF", size: "1.0 MB", icon: "file", tone: "purple", category: "ملفات" }, ...state.files]
      };
      persist();
      render();
      toast("تمت إضافة الملف إلى المكتبة.");
      return;
    }
    if (pageKey === "notes") {
      const title = window.prompt("عنوان الملاحظة", "ملاحظة جديدة");
      ...
      state = {
        ...state,
        notes: [{ id: makeId("note"), title, excerpt: "اكتب تفاصيل الملاحظة هنا...", time: "الآن", tone: "purple" }, ...state.notes]
      };
      persist();
      render();
      toast("تم إنشاء ملاحظة جديدة.");
    ...
    toast("تم تجهيز هذا القسم للاستعراض والتفاعل المباشر.");
  }

  function bindGlobal() {
    $("[data-primary-action]", app)?.addEventListener("click", handlePrimaryAction);
    $("[data-global-search]", app)?.addEventListener("input", (event) => {
      setState({ ...state, search: event.target.value }, false);
    });
    $$(".workspace-icon-btn", app).forEach((button) => {
      button.addEventListener("click", () => toast("هذه نسخة استعراض عامة بدون تسجيل دخول."));
    });
  }

  function render() {
    app.innerHTML = `<div class="workspace-app">${shell(pageConfig.render(state))}</div>`;
    bindGlobal();
