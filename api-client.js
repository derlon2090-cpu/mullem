(() => {
  ...
  function toReadableMessage(value) {
    if (typeof value === "string") {
      ...
      if (message.includes("Invalid value: 'input_text'")) {
        return "ÊÚÐÑ ÇáæÕæá Åáì ÎÏãÉ ÇáÔÇÊ ÇáÂä. ÃÚÏ ÇáãÍÇæáÉ ÈÚÏ Þáíá.";
      }
      if (/authentication is required to use chat/i.test(message)) {
        return "ÇáÔÇÊ ãÊÇÍ ÈÚÏ ÊÓÌíá ÇáÏÎæá ÝÞØ. ÓÌøá ÏÎæáß áÝÊÍ ÇáãÍÇÏËÉ ÏÇÎá ÍÓÇÈß.";
      }
      if (/authentication is required/i.test(message)) {
        return "ÇäÊåÊ ÌáÓÉ ÊÓÌíá ÇáÏÎæá Ãæ áã íÊã ÇáÊÍÞÞ ãäåÇ. ÓÌøá ÏÎæáß ãÑÉ ÃÎÑì ááãÊÇÈÚÉ.";
      }
      if (/admin access is required/i.test(message)) {
        return "åÐå ÇáÚãáíÉ ãÊÇÍÉ áÍÓÇÈ ÇáÃÏãä ÝÞØ.";
      }
      if (/the page could not be found/i.test(message)) {
        return "ÊÚÐÑ ÇáæÕæá Åáì ÇáÎÏãÉ ÇáãØáæÈÉ ÇáÂä. ÍÏøË ÇáÕÝÍÉ Ãæ ÃÚÏ ÇáãÍÇæáÉ ÈÚÏ Þáíá.";
      }
      if (/static hosting detected/i.test(message)) {
        return "ÊÚÐÑ ÇáæÕæá Åáì ÎÇÏã ÇáãæÞÚ ÇáÂä. ÍÏøË ÇáÕÝÍÉ Ãæ ÃÚÏ ÇáãÍÇæáÉ ÈÚÏ Þáíá.";
      }
      if (/getState|cannot read propert/i.test(message)) {
        return "ÊÚÐÑ ÊËÈíÊ ÌáÓÉ ÊÓÌíá ÇáÏÎæá ÇáÂä. ÃÚÏ ÇáãÍÇæáÉ ÈÚÏ Þáíá.";
      }
      return message;
    }
  ...
  function normalizeLegacyUser(user, existingUser = null) {
    ...
    return {
      ...
      status: user?.status || existing.status || (status === "active" ? "äÔØ" : status),
      activity: user?.activity || existing.activity || "ÊãÊ ãÒÇãäÉ ÇáÍÓÇÈ ãÚ ÇáÎÇÏã"
    };
  }
  ...
  async function verifyAuthSessionAfterLogin(result, session) {
    ...
    if (verified.status === 401 || verified.status === 403) {
      ...
      return {
        ...
        message: verified.message || "ÊÚÐÑ ÊËÈíÊ ÌáÓÉ ÊÓÌíá ÇáÏÎæá. ÃÚÏ ÇáãÍÇæáÉ ÈÚÏ Þáíá.",
  ...
  async function openAiWebSearchV2(payload) {
    const result = await request("/openai-web-search-v2", {
      method: "POST",
      body: payload,
      sameOriginOnly: true
    });
    ...
    if (isRouteMissingResult(result)) {
      return {
        ...result,
        message: "ÎÏãÉ ÇáÈÍË ÇáÐßí ÛíÑ ãÊÇÍÉ Úáì ÇáÎÇÏã ÇáÍÇáí. ÍÏøË äÔÑ ÇáÎÇÏã Ãæ ÊÍÞÞ ãä route /api/openai-web-search-v2."
      };
    }
  ...
  function buildWritingFallbackMessage(payload = {}) {
    ...
    const lines = [
      "ÃäÊ ãÓÇÚÏ ÇáßÊÇÈÉ Ýí Orlixor.",
      "äÝÐ ØáÈ ÇáßÊÇÈÉ ÇáÊÇáí ãÈÇÔÑÉ¡ æÇßÊÈ ÇáäÊíÌÉ ÝÞØ ÈÔßá ãÑÊÈ ææÇÖÍ.",
      `äæÚ ÇáãåãÉ: ${payload.task_type || payload.taskType || "generate"}`,
      `äæÚ ÇáãÍÊæì: ${options.content_type || options.contentType || "äÕ"}`,
      `ÇáÛÑÖ: ${options.purpose || "ÚÇã"}`,
      `ÇáäÈÑÉ: ${options.tone || "ÇÍÊÑÇÝíÉ"}`,
      `ÇááÛÉ: ${options.language || "ÇáÚÑÈíÉ"}`,
      `ÇáØæá: ${options.length || "ãÊæÓØ"}`,
      "",
      `ÇáãæÖæÚ Ãæ ÇáäÕ: ${payload.input_text || payload.inputText || payload.topic || payload.message || payload.text || ""}`
    ];

    const details = payload.details || payload.extra_details || payload.extraDetails || "";
    if (details) {
      lines.push("", `ÊÝÇÕíá ÅÖÇÝíÉ: ${details}`);
    }
  ...
  async function runWritingAssistant(payload) {
    ...
    if (!fallback.ok && isRouteMissingResult(fallback)) {
      return {
        ...fallback,
        message: "ÎÏãÉ ãÓÇÚÏ ÇáßÊÇÈÉ ÛíÑ ãÊÇÍÉ Úáì ÇáÎÇÏã ÇáÍÇáí. ÍÏøË äÔÑ ÇáÎÇÏã Ãæ ÇÝÊÍ áæÍÉ ÇáÃÏæÇÊ ãä ÇáÑÇÈØ ÇáÕÍíÍ."
      };
    }
